...
class _PassengerListState extends State<PassengerList> {
  ...
  Future<void> getData() async {
    var response = await http.get(Uri.parse("http://localhost:3000/passenger/all"));
    if (response.statusCode == 200) {
      var jsonResponse =
          convert.jsonDecode(response.body) as Map<String, dynamic>;
      data.add(jsonResponse["passengers"]);
      print(data[0]);
      if (!data.isEmpty) {
        setState(() {
          isLoad = true;
                            ...
                            child: Card(
                              child: Container(
                                ...
                                child: Column(children: [
                                  Text("${data[0][index]["firstName"]}"),
                                  Text("${data[0][index]["lastName"]}"),
                                  Text("${data[0][index]["contact"]}"),
                                  Text("${data[0][index]["nic"]}"),
                                  Text("${data[0][index]["email"]}"),
                                ]),
                              ),
                            ),
                          );
